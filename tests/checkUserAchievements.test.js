...
describe('checkUserAchievements', () => {
  ...
  test('should simulate achievements for galafis', async () => {
    ...
    expect(loggerInfoSpy).toHaveBeenCalledWith(expect.stringContaining('Achievements for galafis'));
    expect(loggerInfoSpy).toHaveBeenCalledWith(expect.stringContaining('First Repository'));
    expect(loggerInfoSpy).toHaveBeenCalledWith(expect.stringContaining('First Commit'));
  });
  ...
  test('should simulate all achievements for octocat', async () => {
    ...
    expect(loggerInfoSpy).toHaveBeenCalledWith(expect.stringContaining('Achievements for octocat'));
    expect(loggerInfoSpy).toHaveBeenCalledWith(expect.stringContaining('Arctic Code Vault Contributor'));
    expect(loggerInfoSpy).toHaveBeenCalledWith(expect.stringContaining('GitHub Sponsor'));
  });
  ...
  test('should show no achievements for an unknown user', async () => {
    ...
    expect(loggerInfoSpy).toHaveBeenCalledWith(expect.stringContaining('No achievements found for this user (simulated).'));
  });
});
